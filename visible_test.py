...
MODEL_PATH = "/share/project/hcr/transformers/src/ERNIE45_03B_Base"
PRINT_TIME = False
TEST_NUM = 1
PRINT_LOGITS = False
...
def generate(model, tokenizer):
    ...
    for i_tokens, token_hidden_state in enumerate(hidden_states):
        out = {}
        out_file = f"./data_visible/token{i_tokens}_hs_logits_by_layer.pt"
        for i_layer, layer_hidden_state in enumerate(token_hidden_state):
            ...
            out[f"layer{i_layer}_hs"] = last_token_hidden_state
            out_logits = model.lm_head(last_token_hidden_state)
            out[f"layer{i_layer}_logits"] = out_logits
